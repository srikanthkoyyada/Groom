...
@Action(value = "quiz-engg-submit", results = {
		@Result(name = "success", location = "/jsp/tests/quizSubmission.jsp"),
		@Result(name = "invalid.token", location = "/jsp/tests/tests.jsp") })
public class IITExamSubmit extends ActionSupport implements ServletRequestAware {
	...
	public String execute() throws SQLException, ParseException {
		...
		if(Integer.parseInt(request.getParameter("division").toString())==2) { //IIT
			correctMarks=correct;
			negativeMarks = (int)incorrect/(int)4;

		} else if(Integer.parseInt(request.getParameter("division").toString())==4) {   //BITSAT
			correctMarks = correct*3;
			negativeMarks = incorrect;
			
		} else {
			correctMarks=correct;
		}
		marks = (int)correctMarks-(int)negativeMarks;
		total=(int)marks;		
		
		System.out.println(request.getParameter("category").toString());
		System.out.println(request.getParameter("division").toString());
		System.out.println("Correct Answers: "+correctMarks);		
		System.out.println("Incorrect Answers: "+incorrect);
		System.out.println("Negative Marks: "+negativeMarks);
		System.out.println("Marks: "+marks);
		
		quizPoints.setPoints(marks);
		quizPoints.setCateogry(request.getParameter("category"));
		quizPoints.setAnswers(userAnswers.toString());
		quizPoints.setTopicid(Integer.parseInt(request.getParameter("topicid")));
		...
		userService.updateUserSubjectPoints(quizPoints.getCateogry(),
				user.getUserId(), correctMarks);
		userService.updateUserTotalPoints(user.getUserId(), correctMarks);
